...
- Added two MCP connections to LimaCharlie's hosted Streamable HTTP server:
  - `limacharlie` (`https://mcp.limacharlie.io/mcp`), restricted to 176 reviewed read-only tools through the `X-MCP-Tools` header. The server enforces the list on every call. Reads that return credentials, fetch URLs or write files stay off it.
